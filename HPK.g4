...
// Leerzeichen und Zeilenumbrüche ignorieren
WHITESPACE: [ \t\n\r]+ -> skip;

// Eine Zahl besteht aus hintereinander geschriebeneden Zahlen von 0 bis 9
// oder aus 2 hintereinander geschriebeneden Zahlen von 0 bis 9 die mit einem Punkt getrennt sind
// oder die Zahl beginnt mit einem Punkt und endet mit hintereinander geschriebeneden Zahlen von 0 bis 9
NUMBER: ([0-9]+ ([.] [0-9]+)?) | ([.] [0-9]+);

// Ein Buchstable ist ein Groß- oder Kleinbuchstabe von a bis z
LETTER: ([a-z] | [A-Z])+;

// Eine Variable beginnt stets mit hintereinander geschriebenden Buchstaben, diese können auch Zahlen beinhalten und auch mit einer Zahl enden
...
bracketExpression: sign = MINUS? LEFT_BRACKET expression RIGHT_BRACKET;
...
expression:
	...
	| <assoc = right> base = expression ('^' | '**') exponent = expression		# Power
	| dividend = expression '/' divisor = expression							# Division
	| factor = expression '*' factor = expression								# Multiplication
	| <assoc = right> number = expression ('mod' | '%') quotient = expression	# Modulo
